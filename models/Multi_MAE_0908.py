...
class Multi_MAE(nn.Module):
    def __init__(self, config):
        ...
        self.rgb_domains = ['f', 'fr', 'fl'] #, 'b', 'bl', 'br']
        ...
        self.rgb_decoder = {
            domain: SpatialOutputAdapter(
                ...
                depth=2,
                task=domain,
                context_tasks=list(self.rgb_domains),
            )
            for domain in self.rgb_domains
        }
        # for domain in self.rgb_domains:
        #     self.rgb_decoder[domain] = self.rgb_decoder[domain].cuda()
        self.f_decoder = SpatialOutputAdapter(
                num_channels=3,
                dim_tokens=trans_dim,
                dim_tokens_enc=trans_dim,
                stride_level=1,
        ...
        self.fl_decoder = SpatialOutputAdapter(
                num_channels=3,
                dim_tokens=trans_dim,
                dim_tokens_enc=trans_dim,
                stride_level=1,
        ...
        self.fr_decoder = SpatialOutputAdapter(
                num_channels=3,
                dim_tokens=trans_dim,
                dim_tokens_enc=trans_dim,
                stride_level=1,
    ...
    def forward(self, pts, rgb, vis = False, **kwargs): 
        ...
        # 1-2. tokenize images
        B, _, H, W = rgb['f'].shape        
        x_rgb = { domain: self.rgb_tokenizer(tensor) for domain, tensor in rgb.items() }
        
        x_info = self.generate_input_info(input_task_tokens=x_rgb, image_size=(H, W))
        task_masks, ids_keep, ids_restore = self.generate_random_masks(x_rgb, num_encoded_tokens = 98) #128)
        ...
        f_pred = self.f_decoder(encoder_tokens=out,
                input_info=x_info,
                ids_keep=ids_keep,
                ids_restore=ids_restore,)
        fl_pred = self.fl_decoder(encoder_tokens=out,
                input_info=x_info,
                ids_keep=ids_keep,
                ids_restore=ids_restore,)
        fr_pred = self.fr_decoder(encoder_tokens=out,
                input_info=x_info,
                ids_keep=ids_keep,
                ids_restore=ids_restore,)
        
        f_loss = self.rgb_loss_func(f_pred.float(), rgb['f'], mask=task_masks.get('f', None))
        fl_loss = self.rgb_loss_func(fl_pred.float(), rgb['fl'], mask=task_masks.get('fl', None))
        fr_loss = self.rgb_loss_func(fr_pred.float(), rgb['fr'], mask=task_masks.get('fr', None))
        rgb_loss = f_loss + fl_loss + fr_loss
